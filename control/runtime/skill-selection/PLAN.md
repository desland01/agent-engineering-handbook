...
| The rule for every worker | Applied: released as `skill-selection-charts`, digest `7138a7371a57a2d1…`, selected on 2026-09-11 (previous release `gsp-app-completion-kernel-fixes` retained and reselectable) | End to end. The release differs from its predecessor only in the two Chart files plus the path rebinding every install performs — the four project grammars and `runtime.json` are identical once the release path is normalised, the three launcher shims excepted. All three markers compose with the new text, and the `local-task` chart digest `428c8a735067da64…` matches what `nautilus inspect` reports. A GLM 5.3 Flash worker admitted under a capsule bound to this digest, told not to read any file, quoted it back: `Read the catalog at intake, before the first substantive action, and name every method the task needs. The ordinary answer`, and named both new sections. |
| The capsule contract and the terminal verdict | Applied: released as `skill-plan-admitted`, digest `d08ad808b13fcce7…`, selected on 2026-09-11 | Live, both halves. A version 5 worker whose plan named `nautilus-core:precise` and never invoked it ended `blocked`, reason `capsule_skill_plan_uninvoked`, with the skill named; the same capsule with the skill invoked ended `completed` and wrote its deliverable; a version 4 capsule whose required chart was never invoked still ended `completed`. Unit: 122 pass / 0 fail, with the new gate tests red against the unpatched source. The rebuild is attributable: deleting `dist/` and rebuilding the unpatched source reproduced all ten bundles byte-for-byte, so the six that changed changed because of this patch. |
| The Anchor constant `N009` | Not staged, and now the only part of this deliverable that is not live | The gate runs inside `runtime.ts` rather than as an Anchor constant, so the obligation is code, not a user decision recorded in the Anchor. Writing it would need a release that edits `anchor.json`, and nothing yet measures a `skill_plan_invoked` fact at that boundary. |
...
## What the live runs found that the unit tests did not — 2026-09-11

The first version 5 capsule was refused before it ran: `capsule_refused_admission`, gap
`capsule_version`, because the admission Watch constant still read *a supported contract
version (1, 2, 3 or 4)* with a bound of `<= 4`. The parser had been extended and the gate
around it had not. Two more ceilings sat behind it — `capsule-admission.mjs` and
`task-lifetime.mjs` both authorised the no-request-cap opt-in with `version === 4`, which
would have refused an uncapped version 5 run. All three are now raised deliberately, the
Watch statement rewritten to say what version 5 additionally declares, and section 5 of the
test file covers each: it is red against the unpatched source and green here.

The lesson for this deliverable is narrow and worth keeping: a parser that accepts a new
contract version proves nothing about the gates that measure it. A live run found in one
attempt what three green test sections had missed.
